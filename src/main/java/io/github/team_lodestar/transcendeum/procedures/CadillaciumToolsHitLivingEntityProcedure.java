		...
		if ((Math.random() < 0.3)) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if ((((!(entityiterator == entity)) && (!(entityiterator == sourceentity)))
						...
						entityiterator.attackEntityFrom(DamageSource.causeMobDamage((LivingEntity) sourceentity), (float) 4);
						entityiterator.setMotion((entityiterator.getMotion().getX()), 0.3, (entityiterator.getMotion().getZ()));
