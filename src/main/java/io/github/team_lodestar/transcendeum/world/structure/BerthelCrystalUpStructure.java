...
import io.github.team_lodestar.transcendeum.procedures.BerthelCrystalUpAdditionalGenerationConditionProcedure;

import com.google.common.collect.ImmutableMap;

@Mod.EventBusSubscriber
public class BerthelCrystalUpStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 550000) {
						...
						for (int a = 0; a < count; a++) {
							...
							int y = spawnTo.getY();
							int z = spawnTo.getZ();
							if (!BerthelCrystalUpAdditionalGenerationConditionProcedure
									.executeProcedure(ImmutableMap.of("x", x, "y", y, "z", z, "world", world)))
								continue;
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("the_transcendeum", "berthelcrystalup"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("berthel_crystal_up"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("the_transcendeum:berthel_crystal_up"), configuredFeature);
		}
	}
	@SubscribeEvent
